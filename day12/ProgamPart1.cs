	...
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			foreach (string child in nodeConnections["start"].Children)
			{
				...
				BuildPaths(child, currentPath, nodeConnections, paths);
			}
		...
		private static void BuildPaths(string currentNode, List<string> currentPath, Dictionary<string, NodeInfo> nodeConnections, List<List<string>> paths)
		{
			...
			else
			{
				...
				{
					if (!string.Equals(child, "end") && string.Equals(child, child.ToLower()) && currentPath.Contains(child)) continue;
					else {
						var currentPathCopy = currentPath.Select(x => x).ToList();
						BuildPaths(child, currentPathCopy, nodeConnections, paths);
					}
				}
			}
		}
